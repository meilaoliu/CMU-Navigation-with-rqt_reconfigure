...
//动态调参
// 动态调参
void dynamicReconfigureCallback(terrain_analysis::terrainAnalysisConfig &config, uint32_t level) {
  quantileZ = config.quantileZ;
  maxGroundLift = config.maxGroundLift;
  vehicleHeight = config.vehicleHeight;

  ROS_INFO("Updated quantileZ: %f", config.quantileZ);
  ROS_INFO("Updated maxGroundLift: %f", config.maxGroundLift);
  ROS_INFO("Updated vehicleHeight: %f", config.vehicleHeight);
  // 根据需要更新其他动态参数
}

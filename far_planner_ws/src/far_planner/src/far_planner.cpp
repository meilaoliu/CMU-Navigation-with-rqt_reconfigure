...
//动态调参
void FARMaster::dynamicReconfigureCallback(far_planner::far_planner_nodeConfig &config, uint32_t level) {
    this->master_params_.vehicle_height = config.vehicle_height;
    this->master_params_.robot_dim = config.robot_dim;
    this->master_params_.sensor_range = config.sensor_range;
    this->master_params_.terrain_range = config.terrain_range;
    this->master_params_.local_planner_range = config.local_planner_range;
    this->master_params_.is_static_env = config.is_static_env;
    this->master_params_.is_attempt_autoswitch = config.is_attempt_autoswitch;

  ROS_INFO("New vehicle_height: %f",this->master_params_.vehicle_height);
  ROS_INFO("New sensor_range: %f", this->master_params_.vehicle_height);
  ROS_INFO("The is_static_env is now: %s", this->master_params_.is_static_env ? "enabled" : "disabled");
  ROS_INFO("The is_attempt_autoswitch is now: %s", this->master_params_.is_attempt_autoswitch ? "enabled" : "disabled");
  ROS_INFO("New robot_dim: %f", this->master_params_.robot_dim);
  ROS_INFO("New terrain_range: %f", this->master_params_.terrain_range);
  ROS_INFO("New local_planner_range: %f", this->master_params_.local_planner_range);
  // 根据需要更新其他动态参数
}

void FARMaster::Init() {

  /* initialize dynamic_server */
  dynamic_server_.setCallback(boost::bind(&FARMaster::dynamicReconfigureCallback, this, _1, _2));
